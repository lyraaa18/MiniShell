...
class ImprovedMiniShell:
    ...
    def execute_command(self, command):
        ...
        try:
            ...
            elif cmd == "nano" or cmd == "vim":
                self.cmd_edit(args[1:])
    ...
    def cmd_edit(self, args):
        ...
        if not args:
            self.log("Usage: edit <filename>", "error")
            return
        filename = args[0]
        if os.path.isfile(filename):
            try:
                subprocess.run([self.get_text_editor(), filename])
            except Exception as e:
    ...
    def cmd_background(self, args):
        if not args:
            self.log("Usage: bg <command> [args...]", "error")
            return
        ...
        if args[0] == "sleep":
            try:
                ...
                if seconds <= 0:
                    self.log("Sleep duration must be positive", "error")
                    return
            ...
            # Define the background function first
                def background_sleep():
                    try:
                        ...
                        self.log(f"[{job_id}] Sleep completed", "success")
                    except Exception as e:
                        job["status"] = "failed"
                        job["error"] = str(e)
                        self.log(f"[{job_id}] Error: {str(e)}", "error")
            
            # Now create the job and thread
                job = {
                    "id": job_id,
                ...
                thread = threading.Thread(target=background_sleep, daemon=True)
                thread.start()
            
                self.background_tasks.append(job)
                self.log(f"[{job_id}] Background sleep started for {seconds} seconds", "info")
                return
            
            except IndexError:
                self.log("Missing sleep duration", "error")
                return
            except ValueError:
                self.log("Invalid sleep duration", "error")
                return

    def cmd_jobs(self):
        """List all background jobs"""
        if not self.background_tasks:
            self.log("No background jobs running.", "info")
            return
        
        self.log("Background Jobs:")
    # Header without PID column
        self.log("{:<5} {:<12} {:<15} {:<25}".format(
            "ID", "Status", "Start Time", "Command"
        ))
    
        for job in self.background_tasks:
        # Safely display job information
            self.log("{:<5} {:<12} {:<15} {:<25}".format(
                job["id"],
                job["status"],
                job["start_time"],
                job["command"][:20] + "..." if len(job["command"]) > 20 else job["command"],
            ))

    def cmd_foreground(self, args):
        """Bring a background job to the foreground"""
        if not args:
            self.log("Usage: fg <job_id>", "error")
            return
            
        try:
            job_id = int(args[0])
            job = next((j for j in self.background_tasks if j["id"] == job_id), None)
            
            if not job:
                self.log(f"No job with ID {job_id}", "error")
                return
                
            if job["status"] == "completed" or job["status"] == "failed":
                self.log(f"Job {job_id} has already completed", "info")
                return
                
            self.log(f"Bringing job {job_id} to foreground...", "info")
            self.log(f"Command: {job['command']}")
            ...
            # Display output
            if stdout:
                self.log(stdout.decode('utf-8', errors='replace'))
            if stderr:
                self.log(stderr.decode('utf-8', errors='replace'), "error")
                
            self.log(f"Job {job_id} completed with exit code {process.returncode}", 
                    "success" if process.returncode == 0 else "error")
                    
        except ValueError:
            self.log("Invalid job ID. Please provide a numeric job ID.", "error")
        except Exception as e:
            self.log(f"Error: {str(e)}", "error")

    def cmd_kill_job(self, args):
        """Terminate a background job"""
        if not args:
            self.log("Usage: kill <job_id>", "error")
            return
        ...
        try:
            ...
            self.log(f"Job {job_id} terminated", "success")
        except ValueError:
            self.log("Invalid job ID. Please provide a numeric job ID.", "error")
        except Exception as e:
            self.log(f"Error: {str(e)}", "error")
    ...
    def cmd_sleep(self, args):
        if not args:
            self.log("sleep: missing operand", "error")
            return
    
        try:
            seconds = float(args[0])
            if seconds < 0:
                self.log("sleep: interval cannot be negative", "error")
                return
            
            time.sleep(seconds)
            self.log(f"Slept for {seconds} seconds", "success")
        except ValueError:
            self.log("sleep: invalid time interval", "error")
        except Exception as e:
            self.log(f"sleep error: {str(e)}", "error")
...
Search:
  find, search          Find files by name
...
"""
    ...
    def show_about(self):
        ...
        A simple and enhanced command-line interface (CLI) built with Python and Tkinter.
        This application provides basic shell functionalities, background process management,
        and real-time system resource monitoring in a user-friendly graphical environment.

        Developed by: Kelompok 6
        Github : https://github.com/lyraaa18/MiniShell
